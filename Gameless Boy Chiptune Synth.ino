...
#include <MIDI.h>
#include <Oscil.h>
#include "tables/square_gb_int8.h"                                      //NES pulse wavetable at 50% duty cycle (square wave)
#include "tables/pulse_12_gb_int8.h"                                    //NES pulse wavetable at 12.5% duty cycle
#include "tables/pulse_25_gb_int8.h"                                    //NES pulse wavetable at 25% duty cycle
#include "tables/pulse_75_gb_int8.h"                                    //NES pulse wavetable at 75% duty cycle
#include "tables/triangle_gb_int8.h"                                    //NES triangle wavetable
#include "tables/noise_gb_int8.h"                                       //NES noise wavetable
#include <tables/sin2048_int8.h>                                        //sine wavetable

#define CONTROL_RATE 1024                                               //sets rate at which MIDI is read
...
byte intensity, nt, vibrato, ch, pu_width_1=2, pu_width_2=2;
float freq, bend=0, rate = 7.f;

Oscil <SIN2048_NUM_CELLS, AUDIO_RATE> aVibe(SIN2048_DATA);              //generates sine wave for vibrato
Oscil <NOISE_GB_NUM_CELLS, AUDIO_RATE> aNoise(NOISE_GB_DATA);         //noise generator
ADSR<CONTROL_RATE/4, AUDIO_RATE> envNoise;                              //envelope generator for noise

struct Voice{                                                           //generates oscillators for pulse waves and triangle wave
  Oscil<OSC_NUM_CELLS, AUDIO_RATE> osc;                                 // audio oscillator
  ADSR<CONTROL_RATE/4, AUDIO_RATE> env;                                 // envelope generator
  byte note = 0;
  byte velocity = 0;
};
...
void setup() {
  ...
  for(unsigned int i = 0; i < MAX_POLY; i++){
    voices[i].env.setTimes(0,65535,65535,100);                         //sets ADSR levels for melodic voices
    voices[i].env.setADLevels(80,40);}                               //sets volume levels for melodic voices
  envNoise.setTimes(0,65535,65535,100);                                //sets ADSR levels for noise
  envNoise.setADLevels(80,40);                                       //sets volume levels for noise
...
void updateControl(){
  ...
  voices[0].osc.setTable(PULSE_12_GB_DATA);}
  if (pu_width_1==1){
  voices[0].osc.setTable(PULSE_25_GB_DATA);}
  if (pu_width_1==2){
  voices[0].osc.setTable(SQUARE_GB_DATA);}
  if (pu_width_1==3){
  voices[0].osc.setTable(PULSE_75_GB_DATA);}
  if (pu_width_2==0){
  voices[1].osc.setTable(PULSE_12_GB_DATA);}
  if (pu_width_2==1){
  voices[1].osc.setTable(PULSE_25_GB_DATA);}
  if (pu_width_2==2){
  voices[1].osc.setTable(SQUARE_GB_DATA);}
  if (pu_width_2==3){
  voices[1].osc.setTable(PULSE_75_GB_DATA);}                          //-^-changes pulse wave wavetables for first two voices-^-
  voices[2].osc.setTable(TRIANGLE_GB_DATA); 
}
...
int updateAudio(){
  ...
  return (long) (currentSample) + (envNoise.next()*aNoise.next())>>8;   //output audio for voices and noise
}
